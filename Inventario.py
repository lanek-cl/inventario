...
if not os.path.exists("last_updates.txt"):
    with open("last_updates.txt", "w") as file:  # format:  entry, out
        init_last_entry = datetime.strptime(
            Ent_sheet.col_values(2)[-1], "%d/%m/%Y %H:%M:%S"
        )
        init_last_out = datetime.strptime(
            Out_sheet.col_values(1)[-1], "%d/%m/%Y %H:%M:%S"
        )
        # print(init_last_entry)
        file.write(
            init_last_entry.strftime("%d/%m/%Y %H:%M:%S")
            + ","
            + init_last_out.strftime("%d/%m/%Y %H:%M:%S")
        )
...
def main(last_edited_entry, last_edited_out):
    ...
    while True:
        try:
            ...
            edited_entry = datetime.strptime(
                Ent_sheet.col_values(2)[-1], "%d/%m/%Y %H:%M:%S"
            )
            edited_out = datetime.strptime(
                Out_sheet.col_values(1)[-1], "%d/%m/%Y %H:%M:%S"
            )

            if edited_entry > last_edited_entry:
                Entry_t = threading.Thread(
                    target=rq.New_entry, args=(Ent_sheet, Inv_sheet)
                )
                ...
                with open("last_updates.txt", "w") as update_e:
                    update_e.write(
                        edited_entry.strftime("%d/%m/%Y %H:%M:%S")
                        + ","
                        + last_edited_out.strftime("%d/%m/%Y %H:%M:%S")
                    )
                ...
                with open("last_updates.txt", "w") as update_e:
                    update_e.write(
                        last_edited_entry.strftime("%d/%m/%Y %H:%M:%S")
                        + ","
                        + edited_out.strftime("%d/%m/%Y %H:%M:%S")
                    )
